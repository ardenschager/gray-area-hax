...
const shellVert = `
    ...
    void main() {
        v_UV = uv + 0.5 * sin(vec2(u_Time * 0.0002, u_Time * 0.00001)) + vec2(0.5);
        v_VertPos = position;
        vec3 pos = position + normal * 0.3 * snoise(position * u_Time * 0.00002);
        vec4 viewPos = modelViewMatrix * vec4( pos, 1.0 );
        v_Pos = projectionMatrix * viewPos;
        // v_Pos += normal * length(vec2(0.5) - v_Pos.xy);
        gl_Position = v_Pos;
    }
`;
...
const shellFrag = `
    ...
    void main () {
        ...
        posRGB = mix(posRGB, sin(u_CameraPos * 0.2), 0.5);
...
let shellMaterial = new THREE.ShaderMaterial( {
	uniforms: {
		u_Time: { value: 0.0 },
    ...
    setMaterialProps: function() {
        ...
        mesh.traverse(function (node) {
            if (node.material) {
                ...
                node.material = shellMaterial.clone();
